 ...
 */
@WebFilter(dispatcherTypes = { DispatcherType.REQUEST }, urlPatterns = { "/templates/*" }, initParams = {
...
public class TemplatesFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (u == null) {
			...
			// Si el usuario no es administrador, redireccion a la pagina inbox
			res.sendRedirect(req.getContextPath() + loginForm);
			return;
		}else if (u.getIsAdmin()) {
			String loginForm = config.getInitParameter("AdminParam");
			// Si el usuario es administrador, redireccion a la pagina principal administrador
			res.sendRedirect(req.getContextPath() + loginForm);
			return;
		}
		chain.doFilter(request, response);
